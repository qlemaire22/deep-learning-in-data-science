...
from dataset import char_to_ind, ind_to_char
...
class RNN:
    ...
    def synthezise_sequence(self, x0, h0, n, b, c, W, U, V):
        Y = np.zeros((self.K, n))
        x = x0
        h = h0
    ...
    def fit(self, book_data):
        ...
        for i in range(self.nb_epochs):
            ...
            for j in range(nb_seq):
                ...
                if ite % 10000 == 0:
                    Y_temp = self.synthezise_sequence(X[:, [0]], hprev, 200, self.b, self.c, self.W, self.U, self.V)
                    string = ""
        ...
        Y_temp = self.synthezise_sequence(char_to_ind("H", self.char_list).T, self.h0, 1000, self.b, self.c, self.W, self.U, self.V)
        string = ""
        for i in range(Y_temp.shape[1]):
            string += ind_to_char(Y_temp[:, [i]], self.char_list)
